...
perl -pi -e 's/^(.*Updated: ).*(.)$/$1$ENV{'DRV'}\x27/' ${H}/chapters/INDEXED/000-Intro.md
perl -pi -e 's/^pagetitle: .THOLONIA.*/pagetitle: \x27THOLONIA $ENV{'VV'}\x27/' ${H}/chapters/000-Intro.md
perl -pi -e 's/version:.*/version:  $ENV{'VV'}/'  ${H}/chapters/INDEXED/000-Intro.md
# perl -pi -e 's/version:.*/version:  $ENV{'VV'}/'  ${H}/chapters/000-Intro.md

exit

#BROKEN# perl -pi -e 's/^(.*Updated: ).*(.)$/$1$ENV{'DRV'}\x27/' ${H}/inc/metadata_epub.yaml
#BROKEN# perl -pi -e 's/^pagetitle: .THOLONIA.*/pagetitle: \x27THOLONIA $ENV{'VV'}\x27/' ${H}/inc/metadata_epub.yaml

#update vesion date _layouts/default.html
#BROKEN# perl -pi -e 's/current version:.*/current version: $ENV{'DRV'}/' ${H}/docs/_layouts/default.html
