...
def update_ver():
    ...
    # also update the titlepage image - REMOVED
    # cx = f'''
    #     convert \
    #     -pointsize 80 \
    #     -font Times-Bold \
...
def html2pdf(chapter):
    ...
    cx += f" --style={H}/Styles/common_book{STATE}.css"# try:
...
def md2epub(chapter):
    ...
    cx += f" --to=epub3"
    cx += f" --epub-cover={H}/Images/publish/6.69x9.61_paperback/i6.69x9.61_Front-Only.jpg"
#    cx += f" --epub-cover={H}/Images/publish/epub/i6.69x9.61_Front-Only-1.jpg"
    cx += f" --standalone"
    cx += f" --mathjax"
    # cx += f" --self-contained"
...
def pub_pdf():
    ...
    cx = f'''\
    cp {H}/chapters/THOLONIA_THE_BOOK.pdf /tmp/tmp.pdf
    # pdftk /home/jw/store/books/tholonia/Images/publish/epub/PDF-cover.pdf /tmp/tmp.pdf cat output {H}/Latest/THOLONIA_THE_BOOK.pdf    
    # pdftk /home/jw/books/tholonia/Images/publish/6.69x9.61_paperback/i6.69x9.61_Front-Only.pdf  /tmp/tmp.pdf cat output {H}/Latest/THOLONIA_THE_BOOK.pdf    
